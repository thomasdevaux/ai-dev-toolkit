...
1. **Get data.** Run the bundled analyzer (default window: last 7 days; honor a different range if the user passed one, e.g. `1h`, `4h`, `24h`, `30d`, or `all` — any `<N>h`/`<N>d` value works, so a short end-of-session window like `1h` or `4h` is as valid as a weekly one). The script `analyze-sessions.mjs` lives in the same directory as this SKILL.md — use its absolute path:
   ```sh
...
- For a just-finished session, `--since 1h` or `4h` (or whatever roughly brackets the session) gives a tighter, more actionable report than the 7-day default — most sessions don't overlap enough with the rest of the week to make the wider window useful for that use case.
