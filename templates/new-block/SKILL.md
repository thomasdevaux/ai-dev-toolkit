...
description: Scaffold a new block for this toolkit repo (org convention, project-type profile variant, or tech-stack convention), following the block-first layout and sync-manifest.yaml registration this repo uses. Use when the user wants to add support for a new tech stack, project type, or org-wide convention.
...
1. **Ask the required questions** if not already answered by the user:
   ...
   - Location: `common/` (org-wide, singleton), `project-type/<variant>/` (a
     new mutually-exclusive project-type profile — confirm the full set of
   ...
   - `tier` for its manifest entry: `baseline` (expected everywhere),
     `choice-group:<name>` (project-type variants), `tech-stack` (with a
...
2. **Create the directory skeleton** under the chosen location:
   ...
   `.claude/` uses), which is why a single manifest entry with
...
4. **Write on-demand workflow skills only for genuine procedures**: name
   ...
   This step is deliberately thin on *how* to write the body well — that's
   the `skill-creator` plugin's job, and the two compose: this skill places
   and registers the block, `skill-creator` writes and tunes the
   `SKILL.md`. Install it once per machine with `python -m tools.sync sync
   skill-creator-user --toolkit-root . --user`. Evaluate the result from
   the scratch project of step 8, not from the block directory — its eval
   loop needs a skill the harness can actually invoke.
...
7. **Register the block** in its own `<location>/manifest.yaml` (create
   the file if the location is new): add one `type: file` entry with
   `id`, `source: <location>`, `target: .claude/`, `scope`, `tier`,
   `summary`, and `detect` if it's a tech stack. Add a separate `type:
   ...
   rather than bundling it into the block's own entry. If the location is
   new, add `- include: <location>/manifest.yaml` to the root
   `sync-manifest.yaml`, in the same order the layout table in AGENTS.md
   lists the directories — the root file is an index of includes, not a
   place to add entries directly.
...
8. **Self-check before finishing**:
   ...
   - Run `python -m tools.audit --toolkit-root .` from the repo root —
     it checks `paths:` overlap against every other block, the character
     budgets on `rules/*.md` (per file, and across the set a project
     syncs), and choice-group integrity in `sync-manifest.yaml`. Fix
     anything it reports.
