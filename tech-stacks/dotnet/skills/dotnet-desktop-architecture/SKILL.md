...
description: Choose, scaffold, and audit the architecture of a .NET desktop app built with Avalonia (C# + XAML). Use whenever the user is building, structuring, or reviewing an Avalonia or .NET desktop application, asks how to organize a C#/XAML desktop codebase, asks about MVVM structure, asks how to publish or distribute a .NET desktop app, mentions Native AOT, trimming, self-contained deployment, or dotnet publish for a desktop app, or needs to integrate a vendor SDK that ships only a .NET/C# library. If the user wants a desktop app but hasn't chosen a stack yet, use the desktop-app-architecture skill first — this skill assumes the .NET branch has already been chosen.
---
