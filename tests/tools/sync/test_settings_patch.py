from tools.sync.settings_patch import (
    declared_hook_commands,
    merge_patch,
    remove_hook_command,
    remove_plugin,
    stale_hook_commands,
    stale_plugin_refs,
)

PROJECT_HOOK = '"$CLAUDE_PROJECT_DIR"/.claude/hooks/a.sh'
USER_HOOK = '"$HOME"/.claude/hooks/b.sh'


def _hooks(event: str, *commands: str) -> dict:
    return {"hooks": {event: [{"hooks": [{"type": "command", "command": c} for c in commands]}]}}


def test_declared_hook_commands_collects_across_patches():
    assert declared_hook_commands(
        [_hooks("SessionStart", PROJECT_HOOK), _hooks("SessionEnd", USER_HOOK), {}]
    ) == {PROJECT_HOOK, USER_HOOK}


def test_stale_hook_commands_only_flags_toolkit_managed_paths():
    settings = _hooks("SessionEnd", PROJECT_HOOK, "./scripts/mine.sh")
    assert stale_hook_commands(settings, set()) == [PROJECT_HOOK]


def test_stale_hook_commands_ignores_still_declared():
    assert stale_hook_commands(_hooks("SessionStart", PROJECT_HOOK), {PROJECT_HOOK}) == []


def test_remove_hook_command_drops_the_containers_it_empties():
    """An empty group or event left behind is inert but reads as a hook that
    exists and does nothing."""
    settings = _hooks("SessionEnd", PROJECT_HOOK)
    assert remove_hook_command(settings, PROJECT_HOOK) == {}


def test_remove_hook_command_keeps_siblings():
    settings = _hooks("SessionStart", PROJECT_HOOK, USER_HOOK)
    result = remove_hook_command(settings, PROJECT_HOOK)
    assert result["hooks"]["SessionStart"][0]["hooks"] == [{"type": "command", "command": USER_HOOK}]


def test_remove_hook_command_preserves_the_matcher_of_a_kept_group():
    settings = {
        "hooks": {
            "PostToolUse": [
                {"matcher": "AskUserQuestion", "hooks": [{"type": "command", "command": PROJECT_HOOK}]},
                {"matcher": "AskUserQuestion", "hooks": [{"type": "command", "command": USER_HOOK}]},
            ]
        }
    }
    result = remove_hook_command(settings, PROJECT_HOOK)
    assert result["hooks"]["PostToolUse"] == [
        {"matcher": "AskUserQuestion", "hooks": [{"type": "command", "command": USER_HOOK}]}
    ]


def test_stale_plugin_refs_filters_known_and_falsy():
    settings = {
        "enabledPlugins": {
            "gitlab@claude-plugins-official": True,
            "old-plugin@some-marketplace": True,
            "disabled-plugin@x": False,
        }
    }
    known = {"gitlab@claude-plugins-official"}
    assert stale_plugin_refs(settings, known) == ["old-plugin@some-marketplace"]


def test_stale_plugin_refs_sorted_when_multiple():
    settings = {
        "enabledPlugins": {
            "zzz@x": True,
            "aaa@x": True,
        }
    }
    assert stale_plugin_refs(settings, set()) == ["aaa@x", "zzz@x"]


def test_stale_plugin_refs_no_enabled_plugins_key():
    assert stale_plugin_refs({}, {"gitlab@claude-plugins-official"}) == []


def test_remove_plugin_deletes_key_only():
    settings = {
        "enabledPlugins": {
            "gitlab@claude-plugins-official": True,
            "old-plugin@some-marketplace": True,
        },
        "otherKey": "untouched",
    }

    result = remove_plugin(settings, "old-plugin@some-marketplace")

    assert result == {
        "enabledPlugins": {"gitlab@claude-plugins-official": True},
        "otherKey": "untouched",
    }
    assert "old-plugin@some-marketplace" in settings["enabledPlugins"]  # original untouched


def test_remove_plugin_missing_key_is_noop():
    settings = {"enabledPlugins": {"gitlab@claude-plugins-official": True}}
    result = remove_plugin(settings, "not-there@x")
    assert result == settings


def test_merge_patch_concatenates_hooks_session_start_from_two_blocks():
    existing = {
        "hooks": {
            "SessionStart": [
                {"hooks": [{"type": "command", "command": "project-type-app hook"}]},
            ]
        }
    }
    patch = {
        "hooks": {
            "SessionStart": [
                {"hooks": [{"type": "command", "command": "toolkit-self-check hook"}]},
            ]
        }
    }

    merged = merge_patch(existing, patch)

    assert merged["hooks"]["SessionStart"] == [
        {"hooks": [{"type": "command", "command": "project-type-app hook"}]},
        {"hooks": [{"type": "command", "command": "toolkit-self-check hook"}]},
    ]


def test_merge_patch_is_idempotent_for_hooks_session_start():
    existing = {
        "hooks": {
            "SessionStart": [
                {"hooks": [{"type": "command", "command": "toolkit-self-check hook"}]},
            ]
        }
    }
    patch = {
        "hooks": {
            "SessionStart": [
                {"hooks": [{"type": "command", "command": "toolkit-self-check hook"}]},
            ]
        }
    }

    merged = merge_patch(existing, patch)

    assert merged["hooks"]["SessionStart"] == [
        {"hooks": [{"type": "command", "command": "toolkit-self-check hook"}]},
    ]


def test_merge_patch_still_replaces_non_list_nested_values():
    existing = {"statusLine": {"type": "command", "command": "old"}}
    patch = {"statusLine": {"type": "command", "command": "new"}}

    merged = merge_patch(existing, patch)

    assert merged["statusLine"] == {"type": "command", "command": "new"}
