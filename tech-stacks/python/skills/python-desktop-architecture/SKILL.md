...
description: Choose, scaffold, and audit the architecture of a Python desktop app (PySide6 or tkinter). Use whenever the user is building, structuring, or reviewing a Python desktop application, asks how to package or distribute one on Windows/macOS/Linux, is worried about download size, startup time, antivirus false positives, or protecting their source code in a shipped Python app, asks about PyInstaller or Nuitka or Cython for packaging, or wants to organize interface code, background work, and business logic in a Qt or tkinter app. If the user wants a desktop app but hasn't chosen a stack yet, use the desktop-app-architecture skill first — this skill assumes the Python branch has already been chosen.
---
