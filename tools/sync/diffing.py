...
def _iter_source_files(source_root: Path):
    ...
    for path in sorted(source_root.rglob("*")):
        if path.is_file():
            yield path, str(path.relative_to(source_root)).replace("\\", "/")


def plan_file_changes(source_root: Path, target_root: Path) -> list[FileChange]:
    ...
    for source_file, relpath in _iter_source_files(source_root):
