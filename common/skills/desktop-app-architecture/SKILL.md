...
description: Choose a technical stack for a new desktop application or internal tool, or audit whether an existing app's stack — or a whole portfolio of tools — still fits. Use when no stack has been settled on yet, or when what the app is or who receives it has changed since it was picked. If the stack is already chosen, use the matching specialized skill instead.
---
...
Useful plain-language framing — what each branch *is*, and what it costs.
When it's the right one is "Choosing a branch" below; don't read both out.

- **Python-based** — "You write everything in Python, including the interface. The trade-off is that the finished app is a fairly large download and starts a bit slowly, and some antivirus software is suspicious of Python-packaged executables."
...
- **.NET-based** — "Microsoft's ecosystem, using C#. Produces a small, fast, self-contained application, with strong support for data-heavy interfaces."
...
| Ecosystem | Larger community, more plugins, more prior art | Smaller but solid. v3 is in beta with a stable desktop API and production apps on it; that's the version the specialized skill covers |
...
| Mobile | Supported | Supported in v3 (iOS and Android build targets), newer and less proven than Tauri's |

If the team is closer to C and wants minimal new-language friction, say that Wails is the lower-friction pick — but let the user confirm rather than deciding for them. If mobile is a firm requirement rather than a maybe, Tauri's support is the more proven of the two. Note that learning curve and build speed compound for a team still learning: more failed attempts, each costing a slower rebuild.
...
This skill ships with `project-type-app`, so it's there as soon as the project
is typed — before `pyproject.toml`, a `Cargo.toml` next to `src-tauri/`, a
`go.mod` next to a Wails `build/config.yml`, or a `.csproj` exists. The specialized skills
live in their **stack blocks**, which stack detection can't suggest yet at
that point. So the handoff is explicit: naming the skill isn't enough, the
block has to be synced first.

Once a branch is chosen, tell the user which block to sync:
...
Give the user the command to run themselves, with the entry id from the table:

```
export CLAUDE_PROJECT_DIR="${CLAUDE_PROJECT_DIR:-$(pwd)}"
source "$CLAUDE_PROJECT_DIR/.claude/hooks/resolve-toolkit-root.sh"
python -m tools.sync sync --toolkit-root "$(resolve_toolkit_root)" \
...
After the sync, the specialized skill is available and takes over.
